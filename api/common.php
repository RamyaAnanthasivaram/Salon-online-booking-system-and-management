...
if(isset($Action) && $Action == "login"){
    ...
    try {
        ...
        $email		= addslashes((trim($_REQUEST['owneremail'])));
        $password	= addslashes((trim($_REQUEST['ownerpassword'])));
    ...
    try {
        ...
        $email		= addslashes((trim($_REQUEST['staffemail'])));
        $password	= addslashes((trim($_REQUEST['staffpassword'])));
    ...
    try {
        ...
        $Query      = "SELECT * FROM tbl_services WHERE category = '$category' AND store = '$store' AND $service = 'Y'";
        $Results    = mysqli_query($conn,$Query);
        ...
        if (mysqli_num_rows($Results) > 0) 
        {
            while($record = mysqli_fetch_assoc($Results)) 
            {
                ...
                if($LoginState)
                {
                    ...
                    {
                        $CartContent = '<button type="button" style="display:none;background-color: deeppink;outline: none;border: none;" class="btn btn-secondary btn-sm" id="bt_cart_'.$record["id"].'" onclick="add_to_cart(\''.$store.'\',\''.$record["id"].'\',\''.$_SESSION["uid"].'\',\''.$type.'\')" >Add to cart</button>
                        ...
                        $CartContent = '<button type="button" style="background-color: deeppink;outline: none;border: none;" class="btn btn-secondary btn-sm" id="bt_cart_'.$record["id"].'" onclick="add_to_cart(\''.$store.'\',\''.$record["id"].'\',\''.$_SESSION["uid"].'\',\''.$type.'\')" >Add to cart</button>
        ...
        if (mysqli_num_rows($Results) > 0) 
        {
            while($record = mysqli_fetch_assoc($Results)) 
            {
                ...
                if($LoginState)
                {
                    ...
                    {
                        $CartContent = '
                        <button type="button"  id="bt_mycart_remove_'.$record["id"].'" onclick="remove_from_mycart(\''.$store.'\',\''.$record["id"].'\',\''.$_SESSION["uid"].'\')" style="display:inline-block;outline:none;border: none;background: #fff;"><img class="img-fluid" style="border-radius: 10px;height: 28px;" src="images/icons/remove.png"></button>';
                    }else{
                        $CartContent = '
                        <button type="button" id="bt_mycart_remove_'.$record["id"].'" onclick="remove_from_mycart(\''.$store.'\',\''.$record["id"].'\',\''.$_SESSION["uid"].'\')" style="display:none;outline:none;border: none;background: #fff;"><img class="img-fluid" style="border-radius: 10px;height: 28px;" src="images/icons/remove.png"></button>';
                    }
...
}else if(isset($Action) && $Action == "delete_store"){
    try {

        $store_id	            = addslashes((trim($_REQUEST['store_id'])));

        $UpdateOrders = "UPDATE tbl_stores SET is_deleted = '1' WHERE id = '$store_id'";
        $UpdateQuery = mysqli_query($conn,$UpdateOrders) or die ("Error in query: $UpdateOrders. ".mysqli_error($conn));

        $ResponseArray["status"]    = "200";
        $ResponseArray["message"]   = "Store deleted.";
